...
| 2.2 Fix the two-part metric | 🟡 PARTIAL | **Decision established:** Gupta = class-agnostic (detection only), Kaggle = 32 typed classes (typing only). **Kaggle side of `classes.json` written**: class 0 = `Not_used` (0 instances confirmed), classes 1-32 named (unverified external source, class 6 flagged uncertain). **Now blocked on a design decision, not missing info:** `Agent_Pipeline_Facts.md` §3 confirms the agent's entity ontology is runtime/per-tenant, not a fixed enum — there is no single canonical ontology to compute Kaggle's "coverage %" against. Needs a decision on a representative reference ontology before the typing harness can be built. See CLAUDE.md rule 5 open question. |
| 2.3 Tile to match agent Stage 3 | 🟡 PARTIAL | **Unblocked** — exact params now known from `Agent_Pipeline_Facts.md` §1: 1024×1024 tiles, 205px overlap, 819px stride, uniform grid, title-block carved out (20px margin, reject if >30% of page area), coords in original page-raster `[x0,y0,x1,y1]`. Implementation not yet written. |
| 2.4 Convert to each model's input format | 🔴 TODO | Blocked on base-VLM candidates. Not started. |
| 2.5 Per-model output-format prompt engineering | 🔴 TODO | Candidates still needed, but the target schema is now known (`Agent_Pipeline_Facts.md` §2) — confidence/bbox nested under `provenance.*`, not top-level. Not started. This is flagged as "most of the battle." |
