...
| 3.4 Optional incumbent (Claude) reference column | ✅ DONE | Run for real (2026-07-11) via the Anthropic API — `claude-sonnet-4-6`, confirmed against `PID_Local_Substitution_Spec.md`'s own architecture table as the actual model the production agent uses for this stage. Same 20 test sheets, same tiling/scoring/stitch harness, same unengineered prompt as Molmo2. **Result: precision 0.392, recall 0.369, F1 0.380, 0/127 parse failures (0%).** Reference only per CLAUDE.md — does not change the ground-truth pass bar. Notably scores *below* Molmo2 zero-shot (F1 0.434) on this exact setup — an early signal, not a settled ranking, since neither candidate's prompt has been engineered yet. |
...
| 4.3 Molmo2 zero-shot | ✅ DONE | **Real result, run over all 20 frozen test sheets (127 tiles) via the Phase 3 harness:** precision 0.538, recall 0.364, **F1 0.434 — below the provisional 0.70 pass bar.** Parse failures 10.2% (13/127 tiles). **Strong density-correlation confirmed with real numbers**, not just hypothesized: sheets with 1-2 tiles hit F1 0.88-1.00 (e.g. sheet `0`, `124`, `129` all F1=1.00); sheets with 12-20 tiles / 100+ ground-truth symbols collapse to F1 0.13-0.52 (worst: sheet `233`, 199 GT symbols, F1=0.13). Recall is the primary weakness — Molmo2 finds proportionally far fewer real symbols as scene density increases, precision degrades less severely. This directly updates the earlier "100% zero-shot parse rate looks promising" read from 2.5 — good format reliability does NOT translate to good detection accuracy zero-shot. Fine-tuning (Phase 5, already built and ready) is the next real test of whether this improves. |
| 4.4 Compare with validity gate | 🟡 PARTIAL | Only 1 of 3 candidates run (Molmo2). Per the checklist's own validity gate: Molmo2's result (F1 0.434, 10.2% parse-failure) is a real, usable measurement, not "inconclusive" — parse failures are low enough to trust the score. Full 3-way comparison still blocked on Qwen3-VL/InternVL3 real runs (4.1/4.2), which remain deferred by decision, not by necessity. |
