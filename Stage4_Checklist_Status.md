...
| 2.2 Fix the two-part metric | 🟡 PARTIAL | **Decision established:** Gupta = class-agnostic (detection only), Kaggle = 32 typed classes (typing only) — confirmed in Step 1, matches CLAUDE.md. **Kaggle side of `classes.json` now written** (`Stage4_Phase2_Data_Preparation.ipynb`): class 0 = `Not_used` (confirmed exactly 0 instances), classes 1-32 mapped to candidate names sourced from the dataset author's own GitHub repo (`ch-hristov/p-id-symbols/dataset.yaml` — unverified, not an official Kaggle listing). Cross-checked 2 names against real data: class 0's zero-count and class 21's 3x-frequency both matched their candidate names exactly, giving reasonable confidence in the indexing scheme. Class 6 (`Butterfly_valve`) visually did not match its crops — flagged uncertain in `classes.json`, not fully trusted. **Still blocked:** the actual `-> agent_type` mapping and ontology-coverage % require the real agent's entity ontology, which this repo does not have access to (same blocker as Stage 3 tiling/output schema). Two scoring paths (detection harness on Gupta, typing harness on Kaggle) not yet built. |
